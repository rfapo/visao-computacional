...
def move_edge_detection_section():
    ...
    # Extrair células a partir da seção 1.5
    cells_to_move = module1['cells'][start_index:]
    cells_to_remove = list(range(start_index, len(module1['cells'])))
    
    print(f"✓ Encontrada seção 1.5 no Módulo 1 (índice {start_index}, {len(cells_to_move)} células)")
